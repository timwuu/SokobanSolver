...
#       2020.05.10 Added SearchAdjacentStates()
...
def SearchAdjacentStates( map, state, adj_states, log):
    ...
    for mov in moves:
        ...
        adj_states.append((new_state, mov))
...
def Solve( map, state, goal):
    ...
    if( not SolveDFS( map, state, state.get_hexdigest(), goal, 0, 0, trace, trace_adj_states, log, 100.0)):
...
def SolveDFS( map, state, state_key, goal, depth, total_steps, trace, trace_adj_states, log, progress_slot):
    ...
    if( len(adj_states)):
        mv_progress_slot = progress_slot/len(adj_states)
    else:
    ...
    for i_item, (adj_state, (mov_dir, steps, box_no)) in enumerate(adj_states):

        g_tm_start()
        key = adj_state.get_hexdigest()
        g_tm_add()
        ...
        log.append([box_no, steps, mov_dir, i_item])

        trace[key] = total_steps+steps+1  #depth+1

        #check if meet goal
        if( adj_state.matchGoal(goal)):
            print( "Reach Goals!")
            print( "Depth:", depth+1)
            ...
            if( SolveDFS( map, adj_state, key, goal, depth+1, total_steps+steps+1, trace, trace_adj_states, log, mv_progress_slot)):
                if FIND_ONLY_ONE:
                    return True
